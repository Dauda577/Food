...
type GroupCategory = {
  id: string; name: string; slug: string; leaves: LeafCategory[];
};
type ParentCategory = {
...
// ── Tile ──────────────────────────────────────────────────────────────────────
const Tile = ({ item, onPress }: { item: LeafCategory; onPress: (item: LeafCategory) => void }) => {
...
// ── Section ───────────────────────────────────────────────────────────────────
const Section = ({ group, isLast, onTilePress, onViewAll, t }: {
  group: GroupCategory; isLast: boolean;
  ...
  <View style={[styles.section, !isLast && styles.sectionDivider]}>
    <View style={styles.secHeader}>
      ...
      <TouchableOpacity style={styles.secViewAllBtn} onPress={() => onViewAll(group)}>
...
export default function CategoriesScreen() {
  ...
  // ── Load groups + leaves when parent changes ──────────────────────────────
  useEffect(() => {
  ...
  const handleLeafPress = useCallback(
    (leaf: LeafCategory) => {
      router.push({ pathname: "/products", params: { categoryId: leaf.id, categoryName: leaf.name } });
    },
    [router]
  );
  ...
  const handleViewAll = useCallback(
    (group: GroupCategory) => {
      router.push({ pathname: "/products", params: { categoryId: group.id, categoryName: group.name } });
    },
    [router]
  );
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]}>
      ...
      <View style={styles.header}>
        ...
        {/* ← t("search") used below, header title stays "Categories" as no key exists */}
      ...
      {error ? (
        <View style={styles.errorWrap}>
          ...
          <TouchableOpacity style={styles.retryBtn} onPress={() => setSelectedId(id => id)}>
...
const styles = StyleSheet.create({
  ...
  iconBtn: { width: 40, height: 40, borderRadius: 12, backgroundColor: "#f8fafc", alignItems: "center", justifyContent: "center" },
  searchContainer: { flexDirection: "row", alignItems: "center", gap: 12, paddingHorizontal: 16, paddingVertical: 12, backgroundColor: "#fff", borderBottomWidth: 1, borderBottomColor: "#f1f5f9" },
  searchBox: { flex: 1, flexDirection: "row", alignItems: "center", gap: 10, backgroundColor: "#f8fafc", paddingHorizontal: 14, paddingVertical: 10, borderRadius: 14, borderWidth: 1, borderColor: "#e2e8f0" },
  searchInput: { flex: 1, fontSize: 14, color: "#0f172a", padding: 0 },
  filterBtn: { width: 44, height: 44, borderRadius: 14, backgroundColor: "#f8fafc", alignItems: "center", justifyContent: "center", borderWidth: 1, borderColor: "#e2e8f0" },
  body: { flex: 1, flexDirection: "row" },
  sidebar: { width: SW, backgroundColor: "#f8fafc", borderRightWidth: 1, borderRightColor: "#e2e8f0" },
  sideItem: { alignItems: "center", paddingVertical: 16, paddingHorizontal: 8, position: "relative" },
  sideItemActive: { backgroundColor: "#fff" },
  sideIconWrap: { width: 44, height: 44, borderRadius: 12, backgroundColor: "#fff", alignItems: "center", justifyContent: "center", marginBottom: 8, shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2, elevation: 1 },
  ...
  sideActiveBar: { position: "absolute", left: 0, top: 0, bottom: 0, width: 3, backgroundColor: "#f97316", borderTopRightRadius: 3, borderBottomRightRadius: 3 },
  panel: { flex: 1, backgroundColor: "#fff" },
  featuredBanner: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", backgroundColor: "#667eea", marginHorizontal: 16, marginTop: 16, marginBottom: 8, padding: 20, borderRadius: 20, overflow: "hidden" },
  ...
  featuredBtn: { flexDirection: "row", alignItems: "center", gap: 6, marginTop: 8, paddingHorizontal: 14, paddingVertical: 8, backgroundColor: "rgba(255,255,255,0.2)", borderRadius: 20, alignSelf: "flex-start" },
  featuredBtnText: { fontSize: 12, fontWeight: "700", color: "#fff" },
  seeAllRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 16, paddingVertical: 16, borderBottomWidth: 1, borderBottomColor: "#f1f5f9" },
  ...
  secHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 16, paddingTop: 16, paddingBottom: 12 },
  ...
  tileImgWrap: { width: TILE - 12, height: TILE - 12, borderRadius: 16, overflow: "hidden", backgroundColor: "#f8fafc", marginBottom: 8, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 4, elevation: 2 },
  tileImg: { width: "100%", height: "100%" },
  tileOverlay: { position: "absolute", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.02)" },
